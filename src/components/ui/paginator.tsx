...
const Page = ({page, isActive, pageChange}:{page:number, isActive:boolean, pageChange:any}) =>{
    return <div onClick={()=>{pageChange(page)}} className={`border border-slate-200 p-2 cursor-pointer text-slate-50 font-semibold rounded-md w-8 text-center ${isActive ? 'bg-brand-3' : 'bg-brand-1'}`}>
        {page}
    </div>
}
